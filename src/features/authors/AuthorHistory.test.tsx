...
rs.mock(import("./api/useAuthorRevisions"));
const mockUseAuthorRevisions = rs.mocked(useAuthorRevisions);
...
test("renders author revision history and detail", async () => {
  mockUseAuthorRevisions.mockReturnValue(querySuccess(revisions));
...
test("renders no history when revisions are empty", () => {
  mockUseAuthorRevisions.mockReturnValue(querySuccess({ authorRevisions: [] }));
  render(<AuthorHistory authorId="author-1" />, { wrapper });
  expect(
